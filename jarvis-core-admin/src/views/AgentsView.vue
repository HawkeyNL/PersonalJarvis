    ...
    <div class="tree-card">
      ...
      <div v-for="[group, agents] in groups" :key="group" class="tree-group">
        ...
        <div v-if="expanded.has(group)" class="tree-children">
          <button v-for="agent in agents" :key="agent.id" class="tree-agent" :class="{ selected: selected?.id === agent.id }" @click="selected = agent"><span>{{ agent.name }}</span><StatusBadge :state="agent.state" /></button>
        </div>
      </div>
    </div>
    <aside class="detail-card sticky-detail"><template v-if="selected"><span class="card-label">SAFE AGENT DETAILS</span><h2>{{ selected.name }}</h2><dl><dt>ID</dt><dd>{{ selected.id }}</dd><dt>Group</dt><dd>{{ selected.group }}</dd><dt>State</dt><dd>{{ selected.state }}</dd><dt>Model policy</dt><dd>{{ selected.model_policy ?? "Not declared" }}</dd><dt>Bundle</dt><dd>{{ data?.bundle.id }}</dd></dl><p class="security-note">Prompt/body: never loaded</p></template><div v-else class="empty-state">Select an agent to inspect safe metadata.</div></aside>
  </section>
